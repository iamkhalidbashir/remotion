...
test('Simple expression', () => {
    expect((0, ffmpeg_volume_expression_1.ffmpegVolumeExpression)({
        volume: 0.5,
        multiplier: 1,
        startInVideo: 0,
        fps: 30,
...
test('Really complex volume expression', () => {
    const expectedExpression = "'if(between(t,-0.0167,0.0167),0,if(between(t,0.0167,0.0500),0.247,if(between(t,0.0500,0.0833),0.505,if(between(t,0.0833,0.2167),0.99,if(between(t,0.2167,0.3833),1,0)))))'";
    expect((0, ffmpeg_volume_expression_1.ffmpegVolumeExpression)({
...
test('Should use 0 as else statement', () => {
    expect((0, ffmpeg_volume_expression_1.ffmpegVolumeExpression)({
        ...
        eval: 'frame',
        value: "'if(between(t,0.0833,0.1500),1,if(between(t,-0.0167,0.0167)+between(t,0.0167,0.0833),0,0))'",
    });
});
test('Simple expression - should not be higher than 1', () => {
    expect((0, ffmpeg_volume_expression_1.ffmpegVolumeExpression)({ volume: 2, multiplier: 1, startInVideo: 0, fps: 30 })).toEqual({
        eval: 'once',
        value: '1',
    });
});
test('Complex expression - should not be higher than 1', () => {
    expect((0, ffmpeg_volume_expression_1.ffmpegVolumeExpression)({
        ...
        eval: 'frame',
        value: "'if(between(t,0.0167,0.0500),1,if(between(t,-0.0167,0.0167),0.505,0))'",
    });
});
test('Should simplify an expression', () => {
    expect((0, ffmpeg_volume_expression_1.ffmpegVolumeExpression)({
        ...
        eval: 'frame',
        value: "'if(between(t,-0.0167,0.0167)+between(t,0.1167,0.1500),0,if(between(t,0.0167,0.1167)+between(t,0.1500,0.1833),1,0))'",
    });
});
...
//# sourceMappingURL=ffmpeg-volume-expression.test.js.map